...
final class LinkedListTests: XCTestCase {
    ...
    func test_LinkedList가_비었을때_enqueue메서드실행시_parameter노드가_firstNode와lastNode가_되는지_확인() {
        //given
        sut?.enqueue(1)
        //when
    ...
    func test_LinkendList에_노드가_존재할때_lastNode의_previousNode프로퍼티가_nil인지_확인() {
        //given
        sut?.enqueue(1)
        //when
    ...
    func test_LinkendList에_노드가_존재할때_enqueue메서드실행시_기존lastNode의_previousNode프로퍼티가_새로들어온파라미터노드가_되는지_확인() {
        //given
        sut?.enqueue(1)
        //when
        sut?.enqueue(2)
    ...
    func test_LinkedList가_비어있을때_dequeue메서드실행시_nil을_반환하는지_확인() {
        //given
    ...
    func test_LinkedList의_노드가_한개일때_dequeue메서드실행시_first노드와lastNode를_nil로_바꿔주는지_확인() {
        //given
        sut?.enqueue(1)
        sut?.dequeue()
        //when
    ...
    func test_LinkedList의_노드가_한개일때_dequeue메서드실행시_firstNode의_value값을_반환하는지_확인() {
        //given
        sut?.enqueue(1)
        //when
        var input:Int = Int()
    ...
    func test_LinkedList의_노드가_여러개일때_dequeue메서드실행시_firstNode의_previousNode를_firstNode로_바꿔주는지_확인() {
        //given
        sut?.enqueue(1)
        sut?.enqueue(2)
        sut?.enqueue(3)
    ...
    func test_LinkedList의_노드가_여러개일때_dequeue메서드실행시_firstNode의_value값을_반환하는지_확인() {
        //given
        sut?.enqueue(1)
        sut?.enqueue(2)
        sut?.enqueue(3)
