...
class CalculatorItemQueueTests: XCTestCase {
    ...
    func test_한개의값을_enqueue로_값이_잘들어가는지() {
        ...
        let input = 2
        //when
        let result = sut.enqueue(element: Double(input))
        //then
    ...
    func test_여러개의값을_enqueue로_값이_순차적으로들어가는지() {
        ...
        let input2 = 3
        //when
        sut.enqueue(element: Double(input1))
        sut.enqueue(element: Double(input2))
        //then
        XCTAssertEqual(sut.enqueueStack, [Double(input1), Double(input2)])
    }
    ...
    func test_큐값이있을때_dequeue로_제일먼저넣어준값이_반환되는지() {
        ...
        sut.enqueue(element: Double(input1))
        sut.enqueue(element: Double(input2))
        sut.enqueue(element: Double(input3))
        //when
