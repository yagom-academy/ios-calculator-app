...
class OperatorTest: XCTestCase {
    ...
    func test_add메서드호출시_마이너스10점5더하기_마이너스30점25값이_40점75가나오는지() {
        let numbers = [10.5, 30.25]
        let operatorValue = Operator(rawValue: "➕")!
        
        sut.operators.enqueue(operatorValue)
        let result = sut.operators.dequeue?.calculate(lhs: numbers[0], rhs: numbers[1])
        
        XCTAssertEqual(result, 40.75)
    }
    
    func test_subtract메서드호출시_352점52345빼기_829점12345값이_마이너스476점6이나오는지() {
    ...
    func test_divide메서드호출시_987점654나누기_123점456값이_8점000048600311041이나오는지() {
    ...
    func test_multiply메서드호출시_123점567곱하기_456점789값이_56444점0463623이나오는지() {
