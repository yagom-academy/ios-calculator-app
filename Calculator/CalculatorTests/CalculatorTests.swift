...
class CalculatorTests: XCTestCase {
    ...
    func switchFunction(_ pushItems: Array<String>) {
        for item in pushItems {
            switch item {
            case "+":
                sut.enqueueBehindNumberOrOperator(Operator(type: .addition))
            case "-":
                sut.enqueueBehindNumberOrOperator(Operator(type: .subtraction))
            case "*":
                sut.enqueueBehindNumberOrOperator(Operator(type: .mulitplication))
            case "/":
                sut.enqueueBehindNumberOrOperator(Operator(type: .division))
            default:
                guard let doubleNumber = Double(item) else {
                    return
                }
                sut.enqueueBehindNumberOrOperator(Operand(value: doubleNumber))
            }
        }
    }
    override func setUp() {
