...
final class ExpressionParserTests: XCTestCase {
    func test_간단하고_정상적인_문자열_수식을_parse_하면_Formula_객체안에_연산자와_피연산자가_분리되어_순서대로_queue에_쌓인다() throws {
        // given
        let input = "5+3"
        var expectation = Formula(operands: [5,3], operators: [.add])
        
        // when
        var result = ExpressionParser.parse(from: input)
        
        // then
        XCTAssertEqual(result.operands.getAllElement(), expectation.operands.getAllElement())
        XCTAssertEqual(result.operators.getAllElement(), expectation.operators.getAllElement())
    }
    
    func test_복잡하고_정상적인_문자열_수식을_parse_하면_Formula_객체안에_연산자와_피연산자가_분리되어_순서대로_queue에_쌓인다() throws {
        // given
        let input = "5+3×2.5÷10−30.14"
        var expectation = Formula(
