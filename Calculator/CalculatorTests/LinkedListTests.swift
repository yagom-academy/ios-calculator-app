...
class LinkedListTests: XCTestCase {
    ...
    func test_빈_LinkedList를_scanAllValue하면_빈_배열을_반환한다() {
        let linkedList = LinkedList<Double>(head: nil)
        XCTAssertEqual(linkedList.scanAllValue(), [])
    }

    func test_1이_있는_LinkedList를_scanAllValue하면_1을_배열에_담아_반환한다() {
        let linkedList = LinkedList<Double>(head: Node(value: 1))
        XCTAssertEqual(linkedList.scanAllValue(), [1])
    }
    
    func test_1과2가_있는_LinkedList를_scanAllValue하면_1과2를_배열에_담아_반환한다() {
        let linkedList = LinkedList<Double>(head: Node(value: 1, next: Node(value: 2)))
        XCTAssertEqual(linkedList.scanAllValue(), [1, 2])
    }
    
    func test_1과2와3과4가_있는_LinkedList를_scanAllValue하면_1과2와3과4를_배열에_담아_반환한다() {
        let linkedList = LinkedList<Double>(head: Node(value: 1, next: Node(value: 2, next: Node(value: 3, next: Node(value: 4)))))
