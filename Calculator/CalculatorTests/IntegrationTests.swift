...
final class IntegrationTests: XCTestCase {
    ...
    func test_정상적인_수식을_parse해서_result를_호출하면_값을_계산해서_반환한다() throws {
        ...
        // when
        var formula1 = try ExpressionParser.parse(from: input1)
        var formula2 = try ExpressionParser.parse(from: input2)
        var formula3 = try ExpressionParser.parse(from: input3)
        var formula4 = try ExpressionParser.parse(from: input4)
        var formula5 = try ExpressionParser.parse(from: input5)
    ...
    func test_0으로_나누는_수식을_parse해서_result를_호출하면_divisionByZero_에러가_발생한다() throws {
        ...
        // when
        let formula1 = try ExpressionParser.parse(from: input1)
        let formula2 = try ExpressionParser.parse(from: input2)
        let formula3 = try ExpressionParser.parse(from: input3)
        let formula4 = try ExpressionParser.parse(from: input4)
        let formula5 = try ExpressionParser.parse(from: input5)
        // then
    ...
    func test_연산자와_피연산자의_순서가_맞지않으면_invalidInput_에러가_발생한다() throws {
        // given
        let input1 = "÷5÷0"
        let input2 = "50+32×100÷+5÷"
        let input3 = "50.1++32.5×10.13--"
        let input4 = "5.153÷0+3.55×-10-"
        let input5 = "--5×10.00÷1"
