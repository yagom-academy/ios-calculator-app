...
class CalculatorItemQueueTests: XCTestCase {
    ...
    func test_enQueue함수를_호출했을때_enQueueStack에_다른값을더하지않는가() {
        //given
        sut?.enQueue(1.0)
        sut?.enQueue(2.0)
        sut?.enQueue(3.0)
        ...
        //then
        XCTAssertFalse(result == sut?.enQueueStack.last)
    }
    ...
    func test_enQueueStack에_값이없다면_deQueue함수를_호출하면_0이나오지는않는가() {
        //given
        var result: Double?
    ...
    func test_deQueue함수를_호출했을때_deQueueStack의배열에서_첫번째값을반환하지는않는가() {
        //given
    ...
    func test_deQueueStack이_비어있을때_enQueue함수를_호출하고_deQueue를호출하면_다른값이들어가지는않는가() {
        //given
        sut?.enQueue(1.0)
        sut?.enQueue(2.0)
        sut?.enQueue(3.0)
