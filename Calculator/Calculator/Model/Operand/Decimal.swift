...
struct Decimal: Operand {
    ...
    static var max: ValueType = pow(10, Double(Constant.numberOfDigits)) - 1
    static var min: ValueType = -(pow(10, Double(Constant.numberOfDigits)) - 1)
    ...
    init?(_ text: String) {
        ...
        if value < 0 {
            guard String(value).count <= (Constant.numberOfDigits + 3) else { return nil }
        }
        guard String(value).count <= (Constant.numberOfDigits + 2) else { return nil }
    ...
    init(_ value: Double) {
        ...
        let count: Int = Constant.numberOfDigits - String(Int(formattedValue)).count
