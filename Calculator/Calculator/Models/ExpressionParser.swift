...
enum ExpressionParser {
    static func parse(from input: String) -> Formula {
        ...
        let operatorType = Operator.allCases.map { String($0.rawValue) }

        let operandComponents = self.componentsByOperators(from: input)
        let operatorComponents = input.map { String($0) }.filter { operatorType.contains($0) }
    ...
    private static func componentsByOperators(from input: String) -> [String] {
        ...
        operatorType.forEach { operatorSet.insert(charactersIn: $0) }
