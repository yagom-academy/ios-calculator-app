...
## STEP 3
### 계산기의 기능 및 조건
-  **AC버튼**: 현재의 모든 연산을 취소하고 0으로 만들기
-  **CE버튼**: 방금 입력한 숫자나 연산자를 취소
-  **+/-버튼**: 부호 변경(0일 때는 변경하지 않음)
-  숫자를 입력하던 중에 연산자를 누르면 다음 숫자를 입력받음
-  현재 입력받은 숫자가 없으면 연산자를 눌러도 계산하지 않고 종류만 변경
-  **=버튼**: 입력된 연산을 수행(연산자 우선순위를 무시 계산)
-  =버튼을 또 눌러도 다시 연산을 하지 않음
-  숫자는 최대 20자리까지 표현(뒤에 의미없는 0은 표시하지 않기)
-  3자리 마다 쉼표 넣기
-  0으로 나누면 **NaN**을 표시

### <br> 코드 설명
- `currentFormula` : 현재까지의 연산을 저장하는 문자열
- `convertToDecimal(_ number: Double) -> String` : 쉼표를 넣은 문자열로 반환하는 함수
- `checkOperatorError()` : 연산자를 넣을 수 있는지 검사하는 함수
- `addFormulaLable(content: String)` : 레이블의 텍스트를 넣어 스크롤뷰에 있는 스택뷰에 넣는 함수 
- `printFormula()` : 현재 입력한 연산자와 피연산자를 스택 뷰에 레이블로 추가하고 스크롤 뷰에 나타내는 함수
- `deleteFormulaLable()` : 현재 스택 뷰에 있는 모든 레이블을 삭제하는 함수
- 그 외 각 버튼별 ACTion 함수들..

### <br> 🧐 고민한 점
#### 1. 현재 계산하는 숫자 표시 방법
처음에 단순히 현재 숫자 레이블에 눌러준 숫자를 추가해주는 방법으로 생각하고 진행하려 했습니다. 숫자를 문자열에 추가하는 것은 쉬웠지만 세자리가 넘어가면 쉼표가 들어가서 다시 새롭게 쉽표를 넣어주기가 어려웠습니다. 또한 계산할 공식에 쉼표가 있는 경우 연산이 불가했습니다. 그래서 계산하고 있는 숫자를 따로 변수로 두면 어떨까 생각해서 진행했고 대부분의 동작을 완성했습니다. 하지만 '.' 버튼 동작에서 구현이 지저분하다는 것을 느끼고 다시 단순히 숫자 문자르 추가하는 방식으로 변경해 숫자를 사용할 때 마다 `filter { $0 != "," }`로 쉼표를 제거했습니다.

#### <br>2. 문자열의 문자 제거 및 사용하기
**CE**버튼으로 마지막 문자를 삭제하거나, 제일 최근에 넣은 문자가 숫자인지 연산자인지 확인하기 위한 경우와 같이 문자열의 마지막 문자를 꺼내고 제거하는 동작을 많이 사용했습니다. 그러면서 비슷한 동작을 하는 함수를 비교하게 되었습니다.

- `last`: 옵셔널 타입으로 마지막 문자를 가져온다.
- `removeLast()`: 마지막 문자를 제거하고 가져온다.
- `popLast()`: 마지막 문자를 제거하고 옵셔널 타입으로 가져온다.
- `dropLast()`: 마지막 문자를 제거한 문자열을 서브스트링으로 반환한다.
